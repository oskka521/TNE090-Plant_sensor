...
void Sensor_process(void)
{
    ...
    if(Sensor_events & SENSOR_READING_TIMEOUT_EVT)
    {
        ...
        if ( tempSensor.objectTemp <= 300){
            Board_Led_control(board_led_type_LED2, 1);
        }
        else{
            Board_Led_control(board_led_type_LED2, 0);
        }
...
static void readSensors(void)
{
    ...
    tempSensor.ambienceTemp = Ssf_readHumiditySensorOskar();
