...
uint16_t OskarsFunc1(uint_fast16_t board1){
        ADC_Params params;
        ADC_Params_init(&params);
        ADC_Handle Myadc = ADC_open(board1, &params);
        if (Myadc == NULL) { while (1);}
        int_fast16_t res;
        uint16_t returnvalue;
        res = ADC_convert(Myadc, &returnvalue);
        if (res != ADC_STATUS_SUCCESS) {while (1);}

        return returnvalue;
}


int16_t Ssf_readHumiditySensorOskar(void){
    int16_t x = OskarsFunc1(Board_ADC1);
    return(x);
}
